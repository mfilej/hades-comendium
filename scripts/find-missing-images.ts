...
function extractImageData(html: string): { hasImg: boolean; hasValidSrc: boolean; src: string | null; dataSrc: string | null; alt: string | null } {
  const parser = new DOMParser();
  const doc = parser.parseFromString(`<div>${html}</div>`, "text/html");
  
  if (!doc) {
    return { hasImg: false, hasValidSrc: false, src: null, dataSrc: null, alt: null };
  }
...
// Get all boons
const boons = db.query<[number, string, number, string, string, string, string, string, string]>("SELECT * FROM boons");

// Array to store missing images info
const missingImages: { id: number; god: string; name: string; img: ReturnType<typeof extractImageData>; iconExists: boolean }[] = [];

// Process each boon
for (const boon of boons) {
  const [id, god, rowIdx, boonName, boonHtml, description, rarity, notes, prerequisites] = boon;
